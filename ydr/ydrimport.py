...
from ..cwxml.drawable import YDR, ShaderItem, ShaderGroupProperty
...
def shader_item_to_material(shader: ShaderItem, shader_group: ShaderGroupProperty, filepath: str):
...
def set_mesh_normals(mesh: bpy.types.Mesh, normals: list[tuple]):
    mesh.polygons.foreach_set("use_smooth", [True] * len(mesh.polygons))
    mesh.normals_split_custom_set_from_vertices(
        [Vector(normal).normalized() for normal in normals])
    mesh.use_auto_smooth = True
...
def drawable_model_to_obj(model, materials, name, lod, bones=None, import_settings=None, armature=None):
    ...
    if import_settings.split_by_group and model.has_skin == 1:
